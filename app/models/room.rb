class Room < ActiveRecord::Base
	def self.individual_search(params)
		...
		sorted_room_prices = room_prices.sort_by{|e| e[1]}.reverse
		...
		rooms.each do |room|
			...
			score += (((sorted_room_areas.index(row) + 0.0) / sorted_room_areas.size) + 2) * 1.2 * params[:pref_size].to_i

			row = sorted_room_prices.detect{|aa| aa.include?(room.id)}
			score += (((sorted_room_prices.index(row) + 0.0) / sorted_room_prices.size) + 2) * 1.2 * params[:pref_price].to_i
